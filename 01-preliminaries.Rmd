...
\begin{pmatrix}
    ...
    \end{pmatrix} = \Omega \begin{pmatrix}
        a_j \\ a_j^\dag 
    \end{pmatrix} = \df 1 {\sqrt{2}}\begin{pmatrix}
    a_j + a_j^\dag \\ 
    (a_j^\dag - a_j)i
    \end{pmatrix}, \quad 
    \Omega 
    = \df 1 {\sqrt 2} \begin{pmatrix}
        1 & 1 \\ -i & i
    \end{pmatrix}
\] 
...
\[ 
    ...
    \quad \{\gamma_j, \gamma_k\} = \delta_{jk}
\] 
The $2n$ majorana operators freely generate a $2^{2n}$-dimensional 
Clifford algebra we denote $\Cl_{2n}$. The basis for $\Cl_{2n}$ are the $2^{2n}$ 
monomials $\gamma_J$ which are at most linear in the generators $\gamma_j$. 
:::
::: {.definition #jw name="Jordan-Wigner transform, ONB"}
Depending on context, it is often more convenient to 
define $\gamma_j\mapsto \sqrt 2\gamma_j$ 
so $q_j=a_j+a_j^\dag, p_j=(a_j^\dag-a_j)i$. In this case 
...
\] 
One may check that this faithfully represents the Clifford algebra 
$\{\gamma_j, \gamma_k\} = 2\delta_{jk}$ in 
...
## Grassmann integrals 
The following two formulas are found in [@knill2001fermionic]. 
